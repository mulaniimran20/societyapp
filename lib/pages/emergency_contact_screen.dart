...
class _MyAppState extends State<MyApp> {
  ...
  void getEmergencyPageDetails() async {
    String urlForEmergencyContactList = urlForApi.emergencyContactDetailsUrl;
    ...
    HttpClientResponse httpClientResponseForEmergencyContactListUrl =
        await getNetworkData.getNetworkDataUsingPostMethod(
            urlForEmergencyContactList, mapSocietyId);
        ...
        for (int i = 0; i < response['loop']; i++) {
          Widget contactCard = ContactDetailsCard(
            cardUserContactNumber: response['data'][0]
                ['emergency_contact_list_contact_number'],
            cardUserName: response['data'][0]
                ['emergency_contact_list_person_name'],
            cardUserEmailId: response['data'][0]
                ['emergency_contact_list_emailid'],
            cardUserPost: response['data'][0]
                ['emergency_contact_list_person_position'],
            color: reusableCardColor,
            cardWithImage: false,
            imageUrl: "https://goviddo.com/logo/male_temp.png",
          );
